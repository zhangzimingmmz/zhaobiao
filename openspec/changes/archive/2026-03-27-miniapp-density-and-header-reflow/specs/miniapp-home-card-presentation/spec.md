...
Bid cards SHALL be composed from direct, supported list fields and SHALL collapse unsupported rows instead of fabricating values. The card presentation SHALL emphasize title, supported key metadata, and weak meta information through a clearer visual hierarchy rather than an older flat list-card appearance, and it SHALL maintain a compact vertical rhythm with restrained local actions so the card does not read as an oversized floating panel.
