...
**自 2026-03-29 起，采集器已实施 IP 消耗优化**：

- **优化策略**：在抓取详情页前，先检查数据库中是否已存在该记录
- **跳过逻辑**：已存在的记录只更新列表页信息，不重复抓取详情页
- **预期效果**：
  - Incremental 任务：节省 50-80% IP（大部分记录已存在）
  - Recovery 任务：节省 80-90% IP（48 小时内记录多已被 incremental 抓取）
  - 年度成本：从约 140 元降低到 30-50 元

**实现细节**：
- 使用 `storage.check_existing_ids()` 批量查询已存在的记录
- 只对新记录调用 `client.fetch_detail()` 抓取详情
- 已存在记录通过 `merge=True` 模式更新，保留原有详情数据
