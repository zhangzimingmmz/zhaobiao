...
项目已有金堂招讯通的 UI 设计（首页列表、详情页、筛选与导航）和 Mock 数据，同时已明确网站一（四川省公共资源交易网）的采集逻辑与可执行模块拆分（SITE1_MODULES.md）。当前缺少：1）可运行的 SITE1 采集管线，将公告数据稳定落库；2）UI 与真实数据源的对接。本变更在「只做稳定原始数据采集、不做正文推导」的前提下，实现 SITE1 全链路采集并将现有 UI 接入真实数据，形成从采集到展示的完整闭环。
