...
const CATEGORY_MAP = {
  plan: '002001009',
  preview: '002001010',
  announcement: '002001001',
}

const FILTER_MODES = {
  plan: 'engineering-plan',
  preview: 'engineering-preview',
  announcement: 'engineering-announcement',
}

const EMPTY_STATES = {
  plan: { title: '暂无招标计划', description: '可以切换发布时间或交易来源后再试。' },
  preview: { title: '暂无招标文件预公示', description: '可以切换发布时间或交易来源后再试。' },
  announcement: { title: '暂无招标公告', description: '可以调整发布时间或交易来源后再试。' },
}
...
export default function ConstructionPage() {
  const isAuthorized = useProtectedPage('请先登录后访问工程建设')
  const [secondary, setSecondary] = useState('plan')
  ...
  const category = CATEGORY_MAP[secondary] || CATEGORY_MAP.plan
  const filterType = FILTER_MODES[secondary] || FILTER_MODES.plan
  const emptyState = EMPTY_STATES[secondary] || EMPTY_STATES.plan
  const isEnd = list.length >= total && total > 0

  const normalizedList = useMemo(() => list.map(normalizeBidItem), [list])

  const hasActiveFilters = useMemo(() => {
    return Boolean(keyword || Object.values(filterValues).some((v) => v && v.code))
  ...
  const handleFilterApply = (key, code, label) => {
    setFilterValues((prev) => ({ ...prev, [key]: { code, label: label || code } }))
  }
    ...
    api.toggleFavorite({ targetId: item.id, targetType: 'bid', targetSite: item.site })
      .then((res) => {
        if (res.data?.code === 200 && res.data?.data) {
          ...
          setList((prev) =>
            prev.map((r) => (r.id === item.id && (r.site || '') === (item.site || '') ? { ...r, favorited: nextFavorited } : r))
          )
  ...
  const filterLabels = useMemo(() => {
    const result = {}
    Object.entries(filterValues).forEach(([k, v]) => { result[k] = v.label })
    return result
  }, [filterValues])
    ...
    if (normalizedList.length === 0) {
      return (
        <EmptyState
          ...
          description={hasActiveFilters ? '可以清空关键词或调整筛选后再试' : emptyState.description}
        />
      )
    }
    return normalizedList.map((item) => (
      <BidCard key={item.id} item={item} onClick={handleCardClick} onFavoriteToggle={handleFavoriteToggle} favorited={!!item.favorited} />
    ))
  }
  ...
  return (
    <View className="page channel-page">
      <TopBar title="工程建设" variant="secondary" showBack />
