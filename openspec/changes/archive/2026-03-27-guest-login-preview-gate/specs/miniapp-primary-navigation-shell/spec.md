...
The miniapp SHALL use three first-level destinations as its primary information architecture: home, favorites, and profile. The home destination SHALL remain a first-level destination, but it SHALL behave as a protected data entry page for authenticated users rather than as a guest-visible landing page.
...
Every miniapp page SHALL belong either to the primary-tab family or to the secondary-page family with back navigation, and protected data pages SHALL remain inaccessible to guests even though they belong to the primary-tab or content page families.
