...
The miniapp SHALL use three first-level destinations as its primary information architecture: home, favorites, and profile. The redesigned shell SHALL strengthen the recognition of those three destinations through a calmer first-level navigation language rather than through oversized gradients, hero blocks, or heavy promotional framing.
...
Every miniapp page SHALL belong either to the primary-tab family or to the secondary-page family with back navigation, and the redesigned shell SHALL keep those families visually distinct while remaining calm and low-chrome.
...
Once favorites and profile are first-level destinations, home SHALL NOT expose them again as global header actions, even after the shell redesign, and the shell SHALL avoid decorative elements that compete with those first-level destinations.
